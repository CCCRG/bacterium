...
def checkCollision(rectBac, food_dic):
    if (
        rectBac["left"] < food_dic["left"] + food_dic["width"]
        and rectBac["left"] + rectBac["width"] > food_dic["left"]
        and rectBac["top"] < food_dic["top"] + food_dic["height"]
        and rectBac["top"] + rectBac["height"] > food_dic["top"]
    ):
        return True
    return False
...
def insert(inter):
    ...
    stxy_all = Edge.objects.values_list("x1", "y1", "x2", "y2")
    ...
    while st == 1 or st == 3:
        ...
        for l in range(0, 4):
            axy1[l] = [
                dr1[l][0] * math.cos(math.radians(r1))
                - dr1[l][1] * math.sin(math.radians(r1))
                + 10
                + x1,
                dr1[l][0] * math.sin(math.radians(r1))
                + dr1[l][1] * math.cos(math.radians(r1))
                + 15
                + y1,
            ]
            axy2[l] = [
                dr1[l][0] * math.cos(math.radians(r2))
                - dr1[l][1] * math.sin(math.radians(r2))
                + 10
                + x2,
                dr1[l][0] * math.sin(math.radians(r2))
                + dr1[l][1] * math.cos(math.radians(r2))
                + 15
                + y2,
            ]
        ...
        rectBac["left"] = xMin
        rectBac["top"] = yMin
        rectBac["width"] = xMax - xMin
        rectBac["height"] = yMax - yMin
        for food in foods:
            food_p = [
                (food["left"], food["top"]),
                (food["left"] + food["width"], food["top"]),
            ...
            if checkCollision(rectBac, food):
                polygon_foods.append(food_p)
                for point_polyg in food_p:
                    points_food.append(point_polyg)
        ...
        if len(polygon_foods) > 0 and len(points) > 0:
            tree = polygons.build_search_tree(
                polygon_foods, num_edges_children, num_nodes_children
            )
            ...
            tree = polygons.build_search_tree(
                polygon_bacterium, num_edges_children, num_nodes_children
            )
        ...
        async_to_sync(channel_layer.group_send)(
            "chat_lobby", {"type": "chat.message", "message": ddd}
        )
...
def rotors(x0, y0, x, y, r):
    ...
    dy = y - y0
    xy = [
        dx * math.cos(math.radians(r)) - dy * math.sin(math.radians(r)) + x0,
...
def distance_e(x1, y1, x2, y2, x3, y3, x4, y4):
    ...
    x = (y01 - y02) / (
        k2 - k1 + 0.00000000000000001
    )  # x точки пересечения линии датчика зрения и линии стены
